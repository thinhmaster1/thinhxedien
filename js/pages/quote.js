...
loadCars().then(cars => {
  ...
  function calculate() {
    ...
    document.querySelector("#quote-results").innerHTML = `<div class="quote-result-head"><div><span>BÁO GIÁ DỰ KIẾN</span><h2>${esc(car.name)}</h2><p>${esc(version.name)} · ${esc(color)}</p>${customer ? `<small>Khách hàng: ${esc(customer)}</small>` : ""}</div><button type="button" id="print-quote">In báo giá</button></div>
