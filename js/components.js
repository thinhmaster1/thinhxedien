import { display, esc, money, UPDATING } from "./core.js?v=2026091903";
...
const THEME_KEY = "thinh-xe-dien-theme";
...
const updateThemeButtons = () => document.querySelectorAll("[data-toggle-theme]").forEach(button => {
...
document.addEventListener("submit", event => {
  if (event.target.id === "private-tools-form") {
    event.preventDefault();
    const input = event.target.querySelector("input");
    const status = event.target.querySelector("[role='status']");
    verifyPrivatePasscode(input.value).then(valid => {
      if (!valid) {
        status.textContent = "Passcode chưa đúng";
        input.value = "";
        input.focus();
        return;
      }
      grantPrivateAccess();
      document.body.classList.add("private-tools-visible");
      status.textContent = "Đã mở công cụ";
    });
    return;
  }
...
export function Footer() {
  return `<footer class="site-footer" id="sources"><div class="site-footer__cta"><div><span>TƯ VẤN VINFAST TẠI BÌNH DƯƠNG</span><h2>Cùng bạn chọn chiếc xe phù hợp.</h2></div><div><a class="footer-call" href="tel:0352978519">Gọi 0352 978 519</a><a class="footer-map" href="https://maps.app.goo.gl/Vfy727dxQVM3tTcp6" target="_blank" rel="noopener">Chỉ đường <span>↗</span></a></div></div><div class="site-footer__main"><div class="site-footer__brand"><a class="brand" href="index.html"><span class="v-mark">T</span><span>Thịnh Xe Điện</span></a><p>Thông tin xe, phiên bản, giá và ưu đãi VinFast được trình bày rõ ràng, dễ tham khảo.</p></div><div class="site-footer__column"><span>SHOWROOM</span><h3>VinFast Bình Thủy<br>Thủ Dầu Một</h3><p>645 Đại lộ Bình Dương<br>08:00–18:30 mỗi ngày</p></div><nav class="site-footer__column" aria-label="Khám phá"><span>KHÁM PHÁ</span><a href="index.html#models">Các dòng xe</a><a href="compare.html">So sánh xe</a><a href="data.html">Bảng giá</a><a class="private-tool-link" href="quote.html">Báo giá</a><a class="private-tool-link" href="loan.html">Tính trả góp</a><a href="policies.html">Khuyến mãi</a></nav><nav class="site-footer__column" aria-label="Kết nối"><span>KẾT NỐI</span><a href="https://www.facebook.com/profile.php?id=61593225476616" target="_blank" rel="noopener">Facebook</a><a href="https://www.tiktok.com/@bui.dac.thinh" target="_blank" rel="noopener">TikTok</a><a href="vinfast-thu-dau-mot-binh-duong.html">Thông tin showroom</a></nav></div><div class="site-footer__bottom"><small>© 2026 Thịnh Xe Điện</small><small>Tư vấn viên tại showroom · Không phải website VinFast chính thức</small></div><div class="private-tools-access"><form id="private-tools-form"><label for="private-tools-passcode">Công cụ nội bộ</label><input id="private-tools-passcode" type="password" autocomplete="current-password" placeholder="Passcode" required><button type="submit">Mở</button><span role="status" aria-live="polite"></span></form><div class="private-tools-links"><a href="quote.html">Mở báo giá <span>›</span></a><a href="loan.html">Mở trả góp <span>›</span></a></div></div></footer>`;
}
