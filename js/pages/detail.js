...
loadCars().then(cars => {
  ...
  document.querySelector("#detail-root").innerHTML = `
    ...
    <section class="story-section"><div><span>HIỆU SUẤT ĐIỆN</span><h2>${car.specs.range ? `Đi xa tới ${car.specs.range.replace(/.*-\s*/,"")}.` : "Sẵn sàng cho mọi hành trình."}</h2><p>Khả năng vận hành được tổng hợp trực tiếp từ tài liệu sản phẩm, giúp bạn đối chiếu từng phiên bản dễ dàng hơn.</p></div><div class="story-visual"><img src="${detailImage}" alt="${car.name} trên đường"><div><span>Công suất</span><b>${car.specs.power || "Đang cập nhật"}</b></div></div></section>
